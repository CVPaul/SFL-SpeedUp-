...
function [outCost,outGrad,covD_Struct] = supervised_WB_CostGrad(U,covD_Struct,beta)

if(covD_Struct.Metric_Flag==1)
    [outCost,outGrad]=compute_AIRM_Sim_CostGrad(U,covD_Struct,beta);

elseif(covD_Struct.Metric_Flag==2)
    [outCost,outGrad]=compute_Stein_Sim_CostGrad(U,covD_Struct,beta);

elseif(covD_Struct.Metric_Flag==3)
    [outCost,outGrad]=compute_KTA_CostGrad(U,covD_Struct);

elseif(covD_Struct.Metric_Flag==4)
    [outCost,outGrad]=compute_LERM_Sim_CostGrad(U,covD_Struct,beta);
else
