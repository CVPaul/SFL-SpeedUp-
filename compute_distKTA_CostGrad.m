...
function [nCost,nGrad]=compute_distKTA_CostGrad(U,trnStruct,beta)
...
%     hK=Compute_Sim_Matrix(Log_UXU);
    hK=Compute_LERM_Sim(Log_UXU,Log_UXU,true,beta);
    hK0 = hK;
    ...
    for i=1:nPoints
        for j=i+1:nPoints
            ...
            nGrad=nGrad+(trnStruct.G(i,j)+trnStruct.G(j,i))*Z_W(i,j)*...
                (-beta)*(hK0(i,j))*(4*XU(:,:,i)*dlogm(UXU(:,:,i),Log_UXU(:,:,i)-Log_UXU(:,:,j))+...
                4*XU(:,:,j)*dlogm(UXU(:,:,j),Log_UXU(:,:,j)-Log_UXU(:,:,i)));
        end
    end
end
