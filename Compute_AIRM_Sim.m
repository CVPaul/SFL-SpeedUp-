...
function [outAIRM, beta] = Compute_AIRM_Sim(Set1,Set2,simFlag,beta)
...
if (simFlag)
    for tmpC1 = 1:l1
        for tmpC2 = tmpC1+1:l2
            ...
            outAIRM(tmpC2,tmpC1) = sqrt(sum(log(tmpEig).^2));
...
if nargin<4
    beta = 1/(2*mean(outAIRM(:))^2);
end
outAIRM=exp(-beta*outAIRM.^2);
end
